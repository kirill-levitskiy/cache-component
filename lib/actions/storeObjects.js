...
exports.process = async function processAction(msg, cfg) {
  ...
  */
  ...
  // create bucket
  const { statusCode: createBucketStatusCode, body: createBucketBody } = await request({
    uri: `${uri}/buckets/`,
    auth: {
      bearer: token,
    },
  ...
  if (createBucketStatusCode !== 201) {
    throw new Error(`Failed to create bucket: ${createBucketStatusCode}: ${createBucketBody}`);
  }
